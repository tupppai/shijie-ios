...
class PPLivesTableViewCell: UITableViewCell {
    ...
    func injectSourceFromModel(model:PPLiveModel?) {
        self.model = model
        
        avatarImageView.sd_setImageWithURL(NSURL(string:(model?.userModel?.avatarUrl) ?? ""))
        liveImageView.sd_setImageWithURL(NSURL(string:(model?.showImageUrl) ?? ""))
        usernameLabel.text  = model?.userModel?.name 
    }
